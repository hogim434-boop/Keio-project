...
- ✅ **Task 015: 게시글 상세 페이지 + 댓글 + 대댓글 (F005, F006, F009)** (의존: Task 014)
  - `app/(app)/posts/[id]/page.tsx` Server Component — Next.js 16 params Promise + fetchPostWithComments + null → `notFound()` + `currentUserId` 추출 + 카테고리 배지·제목·작성자·시간·본문(whitespace-pre-wrap)
  - `components/community/post-detail-actions.tsx` — 낙관적 ❤(up)/👎(down)/🔖 토글 (up↔down 전환 시 양쪽 카운터 보정 6 케이스) + 본인 글 [削除] confirm → DELETE /api/posts/[id] → router.replace('/') + ⋯ placeholder
  - `components/community/{post-detail-thread,comment-list,comment-form}.tsx` — Client wrapper + 1단계 대댓글 트리(ml-6 border-l) + 본인 댓글 削除 + 하단 fixed 폼(z-30) + RHF + zodResolver(CommentFormSchema) + 返信 모드(parentId) + isAnonymous 라디오 default 匿名 + router.refresh 즉시 반영
  - `app/api/comments/[id]/route.ts` DELETE 신규 — params Promise + withAuth + soft delete + RLS 본인/admin
  - 일본어 카피 1:1 (戻る / 削除 / 返信 / 送信 / コメントを入力 / まだコメントがありません / 投稿を削除しました / コメントを削除しました / 反応に失敗しました 등)
  - npm run build 회귀 0건. 잔여: 댓글 추천 토글(Phase 7+), FAB↔폼 z-index 충돌(Task 020 결정). 다음 — Task 016 신고 시트 진입

- ✅ **Task 016: 신고 기능 (F008, W6 와이어프레임)** (의존: Task 015)
  - `lib/stores/report-sheet-store.ts` — Zustand `useReportSheet({ isOpen, target, open, close })`. target = `{ type: 'post' | 'comment', id, preview? }`
  - `components/community/report-sheet.tsx` — 50vh Sheet + RHF + zodResolver(ReportFormSchema) + 사유 라디오 4건(暴言・誹謗中傷 / 名誉毀損 / スパム / 違法・不適切なコンテンツ) + 보충 Textarea 0/500 + 重複 안내 + envelope 처리 (200/409/422/401/기타)
  - 3곳 ⋯ placeholder 통합 — `post-card.tsx` / `post-detail-actions.tsx` / `comment-list.tsx`. 댓글은 본인=削除 / 타인=通報 분기 (타인 댓글에도 ⋯ 노출)
  - `app/(app)/layout.tsx` 에 `<ReportBottomSheet />` 마운트 — 모든 (app) 라우트에서 호출 가능
  - 일본어 카피 W6 1:1 (通報 / 既に通報済みです / 通報を受け付けました 등). 409 UNIQUE → toast.warning + 시트 닫힘
  - npm run build 회귀 0건 (19 라우트). 본인 글 신고 차단은 후속 보강 (현재 본인이라도 통보 메뉴 노출). 다음 — Task 017 탐색·인기 페이지

- ✅ **Task 017: 탐색·인기 페이지 + 검색 (F007, F018)** (의존: Task 014)
  - `components/community/explore-search-bar.tsx` — sticky + 자동 포커스 + 디바운스 300ms + IME composition 가드 + URL `?q` replace + X 클리어 (shadcn Input + lucide Search/X)
  - `components/community/post-feed.tsx` 빈 상태 카피 분기 (search 모드 → 「該当する投稿が見つかりません — キーワードを変えて再検索」 / 평소 → 기존 메시지)
  - `app/(app)/explore/page.tsx` Server Component — searchParams Promise + q 분기 (검색=latest+limit20 / 인기=popular+limit10) + ExploreSearchBar + CategoryIconRow 재사용 + PostFeed key reset
  - 모드별 헤더: 🔥 今週の人気投稿 (q 빈) / 「q」の検索結果 (q 있음)
  - 카테고리 칩 필터 (CategoryIconRow `?category`) + PostCard 인라인 ❤/🔖 그대로 동작
  - npm run build 회귀 0건. 잔여: 인기 7일 윈도우 + pg_trgm similarity 정렬 (P1 후속). 다음 — Task 018 마이페이지

- ✅ **Task 018: 마이페이지 (F009, F011)** (의존: Task 015)
  - `lib/community/my.ts` — `fetchMyPosts` / `fetchMyComments` (post:posts(title) JOIN) / `fetchMyBookmarks` (post + category JOIN). 50건 hard limit
  - `components/community/{my-tabs(sticky 3탭 segment + ?tab), my-post-card(削除), my-comment-card(削除 + 元投稿 링크), my-bookmark-card(🔖 解除), logout-button(signOut)}`
  - `app/(app)/my/page.tsx` Server Component — `?tab` 분기 + 프로필 헤더(닉네임·캠퍼스·학부) + プロフィール編集 링크(`/my/profile` 재활용) + MyTabs + 카드 리스트 + 빈 분기 + 하단 ログアウト
  - 削除/解除/ログアウト → fetch + router.refresh()/replace() 즉시 반영
  - 일본어 카피 W5 1:1
  - npm run build 회귀 0건. Phase 4 (UI) 완료 — Task 012~018. 다음 — Phase 5 어드민 큐 (Task 019)

---

## UI/UX 풀 리뉴얼 (다크 보라/파랑 + Threads 모션) — 횡단 완료

> Phase 1~4 병행으로 진행된 디자인 시스템 리뉴얼. 라우트 구조나 DB 설계에 영향 없이 CSS + 마크업 className 추가만으로 완료.

- **디자인 토큰 (app/globals.css)** — `:root` 다크 보라 토큰 (`--primary: oklch(0.6 0.22 270)`, `--accent: oklch(0.7 0.18 250)`) + 6 utility 클래스
  - `glass-panel` — 반투명 유리 배경 (`backdrop-filter: blur(16px) saturate(140%)` + 테두리)
  - `text-gradient-violet` — 보라→파랑 텍스트 그라데이션
  - `bg-gradient-violet` — 135deg 선형 그라데이션 배경
  - `shadow-glow-violet` — 보라 글로우 박스 섀도우
  - `ring-glow-violet` — 포커스 링 글로우
  - `bg-gradient-violet-radial` — 방사형 그라데이션 배경

- **모션 시스템 (lib/motion-variants.ts)** — 9 신규 export
  - `springTap` / `cardLift` / `heroFadeUp` / `staggerFast` / `fadeInUp` / `glowPulse` / `heartPop` / `sheetSlideUp` / `tabIndicator`

- **랜딩 풀 리뉴얼 (/landing)** — 거대 타이포 + 그라데이션 + stagger 등장 + 3 특징 카드 + CTA glow 버튼

- **핵심 페이지 톤 통일 (/, /posts/[id], /my)** — 글래스 헤더 + 그라데이션 제목 + 보라 accent

- **컴포넌트 모션**
  - `post-card.tsx` — `cardLift` whileHover + `heartPop` 추천 버튼
  - `write-fab.tsx` — `glowPulse` + hover rotate
  - `bottom-tab-bar.tsx` — `layoutId` 슬라이딩 인디케이터
  - `write-bottom-sheet.tsx` — 카테고리 칩 hover 효과
  - 탐색 검색바 — focus ring glow
  - 댓글 목록 — stagger fadeInUp

- **비인증 페이지 톤 통일 (login / signup / signup/setup)** — `glass-panel rounded-2xl` 카드 + `text-gradient-violet` 제목 + `bg-gradient-violet shadow-glow-violet` 버튼
  - `app/(public)/layout.tsx` orb — 보라(`oklch(0.6 0.22 270)`) + 파랑(`oklch(0.7 0.18 250)`) 이미 적용
