...
- **Task 003: 타입 정의 및 데이터베이스 스키마 설계** - 우선순위
  - `types/database.ts`: `users`, `courses`, `reviews` 테이블 TypeScript 인터페이스 선언
  - `types/domain.ts`: 캠퍼스 enum (미타/히요시/SFC/야고이치/시나노마치), 학기 형식, 강의 언어 등 도메인 타입 정리
  - 5축 평점 공통 타입 (`RatingOverall`, `RatingAttendance`, `RatingExamDifficulty`, `RatingGradingEase`, `RatingTeachingStyle`)
  - `supabase/migrations/` SQL 마이그레이션 작성: `users`, `courses`, `reviews` 테이블, RLS 정책, unique(user_id, course_id) 제약, avg_rating 캐시 트리거
  - Supabase MCP `apply_migration`으로 개발 브랜치에 적용 후 `list_tables`로 검증

- **Task 006: 인증 및 회원가입 플로우 구현**
  - Zod로 keio.jp 도메인 이메일 클라이언트 검증 + DB CHECK 제약으로 서버 사이드 이중 검증
  - Supabase Auth `signUp` / `signInWithPassword` / `signOut` 연동
  - 이메일 인증 콜백 처리 → `/courses` 자동 리디렉션
  - 로그인 후 `users` 테이블에 캠퍼스/학부 upsert
  - Playwright MCP 테스트: 잘못된 도메인 차단, 인증 메일 발송, 로그인 성공 리디렉션, 보호 라우트 미인증 차단
